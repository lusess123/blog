# 20250831 Building and prototyping with Claude Code
...
+ 理论上 **RAG** 听起来是个好主意，但它引入了新的（更重要的是，隐藏的）失败模式（failure modes）。要用什么相似性函数？用哪种重排序器（reranker）？如何对代码进行分块（chunk）？对于大型 **JSON** 或日志文件你该怎么办？
